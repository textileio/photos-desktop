          ...
          <Segment basic style={{ overflowY: 'auto', height: 'calc(100vh - 125px)' }}>
            ...
            <Ref innerRef={this.feedRef}>
              <Feed style={{ display: 'flex', flexDirection: 'column-reverse' }}>
                {feed.items.map((item: FeedEvent, index: number) => {
                  return <FeedItem key={item.id} index={index} item={item} onImageClick={this.handleModalOpen} />
                })}
              </Feed>
            </Ref>
            <Modal
              ...
              dimmer
              basic
              content={<LazyImage style={{ maxHeight: '90vh' }} centered src={this.state.src} />}
            />
          </Segment>
        </Ref>
      )
