        ...
        <Header as="h3" style={{ fontFamily: 'Biotif' }}>
          ...
          <Header.Subheader>
            <List divided horizontal>
              <List.Item>
                <Icon name="user" />
                {currentGroup.peer_count}
              </List.Item>
              <List.Item>
                <Icon name="block layout" />
                {currentGroup.block_count}
              </List.Item>
              <List.Item>
                <Icon name="unlock" />
                {currentGroup.type ? currentGroup.type.toLowerCase() : 'private'}
              </List.Item>
              <List.Item>
                <Icon name="share square" />
                {currentGroup.sharing ? currentGroup.sharing.toLowerCase() : 'notshared'}
              </List.Item>
                ...
                <Dropdown item icon={{ name: 'setting' }}>
                  <Dropdown.Menu>
                    <Modal
                      closeIcon
                      size="small"
                      trigger={<Dropdown.Item icon="share square" text="Invite" onClick={this.handleInviteOpen} />}
                      open={store.openInviteModal}
                      onClose={this.handleInviteClose}
                    >
                      <Modal.Header>Invite contacts</Modal.Header>
                      <Modal.Content>
                        <Dropdown
